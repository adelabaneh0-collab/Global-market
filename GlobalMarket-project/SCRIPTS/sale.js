...
const sale=products.filter((product)=>{return product.sale===true})

export function renderSale () {
  let saleHTML='';

  sale.forEach((product)=>{saleHTML+=
    `
   <div class="product-card">
      <div class="product-image-wrap" style="position:relative;">
        <p class="new-arrivals-p"id="">Sale</p>
        <img src="${product.image}" class="products-img">

        <div class="product-hover-buttons">
          <button class="product-action-btn-1"  data-product-id="${product.id}">🛒 ADD TO CART</button>
      ...
      <div class="product-info">
        ...
        <p class="price">$${(product.priceCents / 120).toFixed(2)}</P>
         <p class="price" style="text-decoration:line-through;">$${(product.priceCents / 100).toFixed(2)}</p>
        </div>
      </div>
    </div>
    `
  })
  document.querySelector('.js-sale-grid').innerHTML=saleHTML;
}
...
const productsGrid = document.querySelector(".js-sale-grid");
