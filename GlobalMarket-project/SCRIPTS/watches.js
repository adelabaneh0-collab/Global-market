...
export function renderWatches () {
  const watches=products.filter((product)=>{return product.category==='watches'})
  let watchesHTML='';

  watches.forEach((product)=>{watchesHTML+=
     `
  ...
  document.querySelector('.js-watches-grid').innerHTML=watchesHTML;
}
...
const productsGrid = document.querySelector(".js-watches-grid");
